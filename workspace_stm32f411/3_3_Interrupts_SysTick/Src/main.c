...
void systick_interrupt_delay (uint32_t delay);
...
int main()
{
	...
	while (1)
	{
		systick_interrupt_delay (3000);
		GPIOA->ODR ^= (1U<<5);
		printf("Thats one complete systick cycle \n\r");
	}
...
{
	// Prevent unsigned underflow when the counter reaches 0.
	if (delay_counter > 0U)
	{
		delay_counter --;
	}
